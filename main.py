...
import time
from utils import Results, start, stop
...
parser.add_argument('--mode', choices=['train', 'inference'], default='train', help='training or inference')
...
def main_worker(gpu, ngpus_per_node, args_):
    ...
    for epoch in pbar:
        ...
        acc1, acc5, durations = validate(val_loader, model, criterion, args_, profile=(epoch == 1))

        # set progress bar
        pbar.set_description('Loss: %.3f|Test Acc: %.3f%%' % (loss, acc1))
        # right tensorboard
        result.record(loss, top1_t, top5_t, acc1, acc5, epoch, durations)
